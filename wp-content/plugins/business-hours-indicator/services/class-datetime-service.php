...
namespace MABEL_BHI_LITE\Services
{
	...
	class DateTime_Service
	{
		...
		private function __construct($timeZone)
		{
			...
			$this->daysOfWeekShort = array('Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su');
		...
		public function copy_sets_to_future(array $sets,$offset_in_weeks = 1)
		{
			...
			foreach($sets as $set)
			{
				...
				foreach($set->opening_hours as $hour)
				{
					...
					array_push($new_set->opening_hours,$new_hour);
				}
				array_push($new_sets,$new_set);
			}
			return $new_sets;
		}
		...
		public function get_new_datetime($format)
		{
			return new DateTime($format,$this->timeZone);
		}
