	...
	class Shortcode_Controller
	{
		...
		private function init_indicator_shortcode()
		{
			new Shortcode(
				'mbhi',
				'indicator.php',
				array($this,'create_indicator_model'),
				array(
					'location' => null,
		...
		private function init_isopenclosed_shortcodes()
		{
			new Shortcode(
				'mbhi_ifopen',
				'ifopenclosed.php',
				array($this,'create_ifopenclosed_model'),
				array('location' => null)
			);
			new Shortcode(
				'mbhi_ifclosed',
		...
		private function init_hours_list_shortcode()
		{
			new Shortcode(
				'mbhi_hours',
				'list.php',
				array($this,'create_hours_list_model'),
				array(
					'location' => null,
					'format'=> Settings_Manager::get_setting('format'),
		...
		public function create_hours_list_model($attributes)
		{
			...
			$consolidated = $attributes['display'] == 1;
			...
			if ($model->has_current === 1 && Enumerable::from($model->special_entries)->any('$x => $x->current == true'))
				$model->has_current = 2;
			}

			if($model->show_vacations){
				$model->vacation_entries = $this->vacations_as_output($location, $consolidated );
				if($model->has_current === 1 && Enumerable::from($model->vacation_entries)->any('$x => $x->current == true'))
					$model->has_current = 3;
			}

			$model->show_as_table = $attributes['output'] == 1;
		...
		private function sets_as_consolidated_output(array $sets, $time_format)
		{
			...
			foreach($sets as $set)
			{
				if(sizeof($consolidated_row) === 0) {
					array_push($consolidated_row,$set);
					continue;
				}
				$is_same = true;
				...
				if($is_same){
					...
					array_push($consolidated, $consolidated_row);
					$consolidated_row = array($set);
				}
			}
			array_push($consolidated,$consolidated_row);
			...
			foreach($consolidated as $consolidated_row)
			{
				...
				$has_current = Enumerable::from($consolidated_row)->firstOrDefault('$x => $x->is_today == true');
		...
		private function specials_as_output(Location $location, $consolidated, $time_format)
		{
			$entries = array();
			$closed = __('Closed', $this->slug);
			foreach($location->specials as $special) {
				$entry = new List_VM_Entry();
				$entry->range = sprintf(
					'%s %s',
					$special->date->format('j'),
					__($special->date->format('M'))
				);
			...
			foreach($entries as $entry){
				...
				$consolidated_entry->range = $consolidated_row->join('$x => $x->range',', ');
				$consolidated_entry->hours = $entry->hours;
				$consolidated_entry->current = $consolidated_row->firstOrDefault('$x => $x->current === true') != null;

				array_push($consolidated_entries, $consolidated_entry);
			}
		...
		private function vacations_as_output(Location $location,$consolidated)
		{
			...
			foreach($location->vacations as $vacation)
			{
				...
				$entry->range = sprintf(
					...
					__($vacation->from->format('M')),
					$vacation->to->format('j'),
					__($vacation->to->format('M'))
				);
			...
			$entry = new List_VM_Entry();
			$entry->range = $collection->join('$x => $x->range',', ');
			$entry->current = $collection->firstOrDefault('$x => $x->spans_today === true') != null;
